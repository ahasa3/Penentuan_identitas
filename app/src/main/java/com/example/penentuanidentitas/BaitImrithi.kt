package com.example.penentuanidentitas

class BaitImrithi {
    companion object{
        val muqoddimah = mapOf(
            "arab" to arrayOf(
                "المقدمة",
                "اَلحْـَمْدُ للهِ الَّـذِى قَدْ وَفَقَ * لِلْعِلْمِ خَيْرَ خَلْقِهِ وَللِتُّقَى",
                "فَأُشْرِبَتْ مَعْنَى ضَمِيرِ الشَّانِ * فَأَعْرَبَتْ فِي ألحَانِ بِالأَلْحانِ",
                "ثُمَّ الصَّلاَةُ مَعَ سَلاَمٍ لاَئِقِ * عَلَى النَّبِيِّ أَفْصَحِ الْخَلاَئِقِ",
                "مُحَمَّدٍ وَالآلِ وَالأَصْحابِ * مَنْ أَتْقَنُوا الْقُرْءَانَ بِالإعْرَابِ",
                "وَبَعْدُ فَاعْلَمْ أَنَّهُ لَمَّا اقْتَصَرْ * جُلُّ الْوَرَى عَلَى الْكَلاَمِ المَخْتَصَرْ",
                "وَكَانَ مَطْلُوباً أَشَدَّ الطَّلَبِ * مِنَ الْوَرَى حِفْظُ اللِّسَانِ الْعَرَبي",
                "كَيْ يَفْهَمُوا مَعَانِيَ الْقُرْءَانِ * وَالسُّنَّةِ الدَّقِيقَةِ المَعَانِي",
                "وَالنَّحْوُ أَوْلَى أَوَّلاً أَنْ يُعْلَمَا * إذِ الْكَلاَمُ دونَهُ لَنْ يُفْهَمَا",
                "وَكَانَ خَيْرُ كُتْبِهِ الصَّغِيْرَهْ * كرَّاسَةً لَطِيفَةً شَهِيرَهْ",
                "في عُرْبِهَا وَعُجْمِهَا والرُّومِ * أَلَّفَهَا الْحَبْرُ ابْنُءَاجُرُّومِ",
                "وَانْتَفَعَتْ أَجِلَّةٌ بِعِلْمِهَا * مَعْ ما تَرَاهُ مِنْ لَطِيفِ حَجْمِهَا",
                "نَظَمْتُهَا نَظْماً بَدِيعاً مُقْتَدِي * بِالأَصْلِ في تَقْريبهِ لِلمُبْتَدِى",
                "وَقَدْ حَذَفْتُ مِنْهُ ما عَنْهُ غِنَى * وَزِدْتُهُ فَوَائِداً بِهَا الغِنَى",
                "مُتَمِّماً لِغَالِبِ الأَبْوَابِ * فَجَاءَ مِثْلَ الشَّرْحِ لِلْكِتَابِ",
                "سُئِلْتُ فِيهِ مِنْ صَدِيقٍ صَادِقِ * يَفْهَمُ قَوْلِي لاِعْتِقَادٍ واثِقِ",
                "إذِ الْفَتَى حَسْبَ اعْتِقَادِهِ رُفِعْ * وَكُلُّ مَنْ لَمْ يَعْتَقِدْ لَمْ يَنْتَفِعْ",
                "فَنَسْأَلُ المَنَّانَ أَنْ يُجِيرَنَا * مِنَ الرِّيَا مُضَاعِفاً أُجُورَنَا",
                "وَأَنْ يَكُونَ نَافِعاً بِعِلْمِهِ * مَنِ اعْتَنَى بِحِفْظِهِ وَفَهْمِهِ"
            ),
            "terjemah" to arrayOf(
                "Pendahuluan",
                "Dengan nama Allah yang Maha Pengasih lagi Maha Penyayang. Segala puji bagi Allah yang telah melimpahkan taufiq-Nya kepada hamba-hamba terbaik-Nya dengan ilmu dan Taqwa, Sehingga hati mereka tertuju pada-Nya",
                "Namun mereka tidak mampu mencakupNya karena keagungan Dzat-Nya, Maka dipadukanlah makna dhamir sya’n (yaitu 'Laa ilaaha illallaah') dalam hati hingga bisa merasakan indahnya bermakrifat kepada-Nya serasa mendengarkan alunan lagu",
                "Shalawat serta salam selanjutnya dihaturkan kepada Nabi yang paling fasih lisannya",
                "Yaitu Muhammad SAW., kepada keluarganya, dan sahabatnya yang meyakini Al Qur’an dengan sebab i’rob",
                "Selanjutnya, ketahuilah bahwa karena banyak ulama besar membuat ringkasan dari kitab yang sudah diringkas",
                "Dan mendalami bahasa Arab sangat penting bagi manusia",
                "Agar mereka bisa memahami Al-Qur’an dan As-Sunnah yang rumit kandungan maknanya",
                "Ilmu nahwu lebih berhak dipelajari dahulu, karena kalam Arab tanpa ilmu nahwu tidak akan bisa dipahami",
                "Bahwa sebaik-baik kitab Nahwu yang kecil adalah hanya ada satu kuras yang begitu tipis",
                "Terkenal di negeri Arab dan Ajam, juga di kerajaan Rum. Dan dikarang oleh pakarnya, Ibnu Ajurum",
                "Ulama-ulama besar banyak yang mengambil manfaat dari ilmunya kitab Jurumiyah, padahal mereka tahu betapa kecil bentuknya",
                "Maka aku nazhamkan kitab Jurumiyah dengan susunan yang indah serta mengikuti kitab asalnya dalam memudahkan orang yang mulai belajar ilmu nahwu",
                "Telah aku buang beberapa materi yang kurang perlu dan aku tambahkan beberapa faidah yang penting",
                "Sebagai pelengkap bab-bab yang ada, maka kitab ini seperti sebuah komentar (syarah) dari kitab Jurumiyah",
                "Aku diminta membuat nazham ini oleh beberapa sahabat dekat yang benar-benar bisa memahami ucapanku dengan tekad yang kuat",
                "Karena kemuliaan kaum muda tergantung tekadnya, barang siapa tidak mempunyai tekad yang kuat akan gagal meraih keberhasilan",
                "Aku mohon pada Dzat yang Maha Pemurah agar menyelamatkan diriku dari sifat riya serta melipat gandakan pahalaku",
                "dan semoga memberikan manfaat ilmu nazam ini kepada siapa saja yang telah berusaha menghafalkan dan memahaminya"
                )
        )
        val babKalam = mapOf(
            "arab" to arrayOf(
                "باب الكلام",
                "كَلاَمُهُمْ لَفْظُ مُفِيدٌ مُسْنَدُ * وَالْكِلْمَةُ اللَّفْظُ المُفِيدُ المُفْرَدُ",
                "لاِسْمٍ وَفِعْلٍ ثُمَّ حَرْفٍ تَنْقَسِمْ * وَهَذِهِ ثَلاَثَةٌ هِيَ الْكَلِمْ",
                "وَالْقَوْلُ لَفْظٌ قَدْ أفَادَ مُطْلَقاً * كَقُمْ وَقَدْ وَإِنَّ زَيْداً ارْتَقَى",
                "فَالاِسْمُ بِالتَّنْوِينِ والْخَفْضِ عُرِفْ * وحَرْفِ خَفْضٍ وَبِلاَمٍ وَأَلِفْ",
                "وَالْفِعْلُ مَعْرُوفٌ بِقَدْ وَالسِّينِ * وَتَاءِ تَأْنِيثٍ مَعَ التَّسْكِينِ",
                "وَتَا فَعَلْتَ مُطْلَقاً كَجِئْتَ لِي * وَالنُّونِ وَالْيَا فِي افْعَلَنَّ وافْعَلِي",
                "وَالْحَرْفُ لَمْ يَصْلُحْ لَهُ عَلاَمَهْ * إلاَّ انْتِفَا قَبُولِهِ الْعَلاَمَهْ"
            ),
            "terjemah" to arrayOf(
                "Bab-Kalam",
                "Kalam menurut Nahwu ialah lafaz yang berfaedah dan musnad (murakkab), Sedangkan kalimah ialah lafaz yang berfaidah (mufid), yakni tidak murakkab",
                "Kalimah itu terbagi menjadi tiga: isim, fi’il, dan isim, dan kumpulnya ketiga kalimah itu disebut kalim",
                "Qauf ialah lafaz yang memberi faedah muthalaq. Seperti Qum Qad dan sesungguhnya Zaid naik",
                "Ciri-ciri isim ialah tanwin, jer (hafadl), huruf dan alim lam (ال)",
                "Fi'il itu bisa diketahui dengan Qad, sin, danta ta’nits yang sukun (mati)",
                "Ta’ fa’il secara mutlak, nun taukid (tsaqilah atau kholifah) dan ya’ muannats mukhathabah, seperti contoh افْعَلَنَّ dan افْعَلِي",
                "Huruf itu selamanya tidak diberi tanda, tidak diberinya tanda itu menjadi tanda huruf"
            )
        )
        val babIrob = mapOf(
            "arab" to arrayOf(
                "بَابُ الإعْرَابِ",
                "إِعْرَابُهُمْ تَغْييرُ آخِرِ الْكَلِمْ * تَقْدِيراً أو لَفْظاً لِعَامِلٍ عُلِمْ",
                "أَقْسَامُهُ أَرْبَعَةٌ فَلْتُعْتَبَرْ * رَفْعٌ وَنَصْبٌ وَكَذَا جَزْمٌ وَجرْ",
                "وَالكُلُّ غَيْر الجَزمِ فِي الأَسمَا يَقَعْ * وَكُلُّهَا فِي الْفِعْلِ وَالْخَفْضُ امْتَنَعْ",
                "وَسَائِرُ الأَسْمَاءِ حَيْثُ لاَ شَبَهْ * قَرَّبَهَا مِنَ الحُرُوْفِ مُعْرَبَهْ",
                "وَغَيْرُذِي الأَسْمَاء مَبْنِيُّ خَلاَ * مُضَارِعٍ مِنْ كُلِّ نُونٍ قَدْ خَلاَ"
            ),
            "terjemah" to arrayOf(
                "Bab I'rob",
                "I’rob menurut ahli nahwu ialah berubahnya akhir-akhir kalimah ‘amil (Yang memerintah) yang masuk kepadanya, baik berubah kira-kira maupun lafaznya",
                "Pembagian I’rob itu ada empat macam, yaitu: Rofa’, Nashob, Jazm, dan Jer",
                "Semua i'rob selain Jazm dapat jatuh pada isim, semua irab dapat jatuh di fi'il selain jer",
                "Semua isim yang tiada serupa yang mendekatkan pada huruf, itu hukumnya mu’rob",
                "Selain isim mu’rob itu hukumnya mabni, kecuali fi’il mudhari’ yang sepi dari Nun Taukid, maka hukumnya mu’rob, adapun fi’il mudhari’ yang kemasukan Nun Taukid hukumnya mabni"
            )
        )
        val babTandaRofa = mapOf(
            "arab" to arrayOf(
                "باب علامات الرفع",
                "لِلرَّفْعِ مِنْهَا ضّمَّةٌ وَاوٌ أَلِفْ * َكذَاكَ نُوْنٌ ثَابِتٌ لاَ مُنْحَذِفْ",
                "فَالضَّمُّ فِي اسْمٍ مُفْرَدٍ كَأَحْمَدِ * وَجَمْعِ تَكْسِيرٍ كَجَاءَ الأَعْبُدِ",
                "وَجَمْعِ تَأْنِيثٍ كَمُسْلِمَاتٍ * وَكُلِّ فِعْلٍ مُعْرَبٍ كيَاتِي",
                "وَالْوَاوُفِي جَمْعِ الذُّكُورِ السَّالِمِ * كَالصَّالِحُونَ هُمْ أُولُو المَكَارِمِ",
                "كَمَاأَتَتْ فِي الخَمْسَةِ الأَسْمَاءِ * وَهْيَ الَّتِي تَأْتِي عَلَى الْوِلاءِ",
                "أَبٌ أَخٌ حَمٌ وَفُوكَ ذُو جَرَى * كُلٌّ مُضَافاً مُفْرَداً مُكَبَّرَا",
                "وَفِي مُثَنَّى نَحْوُ زَيْدَانِ الأَلِفْ * وَالنُّونُ فِي المُضَارعِ الَّذِي عُرِفْ",
                "بِيَفْعَلاَنِ تَفْعَلاَنِ أَنْتُمَا * وَيَفْعَلُونَ تَفْعَلُونَ مَعْهُمَا",
                "وَتَفْعَلِينَ تَرْحَمِينَ حَالِي * وَاشْتَهَرَتْ بِالْخَمْسَةِ الأَفْعَالِ"
            ),
            "terjemah" to arrayOf(
                "Bab Tanda-tanda Rofa'",
                "Bagi i’rob Rofa’ ada tanda-tandanya, yaitu: dhommah, wawu, alif, dan nun yang tidak dibuang",
                "Dhommah menjadi tanda rofa’ dalam Isim mufrod seperti Ahmad, Jama’ taksir seperti جَاءَ الأَعْبُدِ",
                "Jama’ muannats salim, seperti Muslimat, dan setiap Fi'il yang mu’rob (mudhari’)",
                "Wawu menjadi tanda rofa’ berada pada jama’ mudzakkar salim, Misalnya seperti الصَّالِحُونَ هُمْ أُولُو المَكَارِمِ",
                "Demikian pula pada Asmaul khomsah, yang akan diterangkan secara beruntut, yaitu:",
                "Semua asmaul khomsah (أَبٌ أَخٌ حَمٌ فُوكَ ذُو) itu harus diidhofahkan dan harus mufrad (bukan tatsniyah atau jama') dan harus serta mukabbar (tidak ditasghirkan)",
                "Alif menjadi tanda rofa’ itu berada pada isim tatsniyah, Nun menjadi tanda rofa’ berada pada fi’il mudhari’ yang dikenah dengan ..",
                "Yafalani dan Tafalani (Fi'il Mudlari yang akhirnya bertemu dengan dhamir alif tatsniyah) Yaf'aluna Taf'aluna (Fi'il mudhari yang akhirnya bertemu wawu jama’)",
                "Taf'aliina (Fi'il Mudari yang akhirnya bertemu ya’ muannats mukhathabah) yang dikenal dengan sebutan Af'aalul khamsah"
            )
        )
        val babTandaNashob = mapOf(
            "arab" to arrayOf(
                "بَابُ عَلاَمَاتِ النَّصْبِ",
                "لِلنَّصْبِ خَمْسٌ وَهْيَ فَتْحَةٌ أَلِفْ * كَسْرٌ وَيَاءٌ ثُمَّ نُونٌ تَنْحَذِفْ",
                "فَانْصِبْ بِفَتْحٍ مَا بِضَمٍّ قَدْ رُفِعْ * إِلاَّ كَهِنْدَاتٍ فَفَتْحُهُ مُنِعْ",
                "وَاجْعَلْ لِنَصْبِ الخَمْسَةِ الأسْمَا أَلِفٌ * وَانْصِبْ بِكَسْرٍ جَمْعَ تَأْنِيثٍ عُرِفْ",
                "وَالنَّصْبُ فِي الاِسْمِ الَّذِي قَدْ ثُنِّيَا * وَجَمْعِ تَذْكِيرٍ مُصَحَّحٍ بِيَا",
                "وَالْخَمْسَةُ الأفْعَالُ حَيْثُ تَنْتَصِبْ * فَحَذْفُ نُونِ الرَّفْعِ مُطْلَقاً يَجِبْ"
            ),
            "terjemah" to arrayOf(
                "Bab Tanda-tanda Nashob",
                "I'rab Nashob itu mempunyai lima tanda, yaitu: Fathah, Alif, Kasrah, Ya’, dan Nun yang dibuang",
                "Nashobkanlah dengan fathah pada setiap lafadz yang dirofa’kan dengan dhommah, kecuali pada lafadz seperti:  هِنْدَاتٍ (Jama’ mu’annas salim)",
                "Nashobkanlah isim lima (asmaul khomsah) dengan alif. Jadi, alif menjadi tanda i’rab nashob itu hanya bertempat pada asmaul khomsah",
                "Ya’ menjadi tanda i’rab nashob itu hanya bertempat pada dua tempat, yaitu: Isim tatsniyah, dan Jama’ mudzakkar salim",
                "Af'alul khomsah ketika tingkah nashob itu diberi tanda dengan membuang nun rofa’"
            )
        )
        val babTandaJer = mapOf(
            "arab" to arrayOf(
                "بَابُ عَلاَمَاتِ الخَفْضِ",
                "عَلاَمَةُ الخَفْضِ الَّتِي بِهَا انْضَبَطْ * كَسْرٌ وَيَاءٌ ثُمَّ فَتْحَةٌ فَقَطْ",
                "فَاخْفِضْ بِكَسْرٍ مَا مِنَ الأَسْمَا عُرِفْ * فِي رَفْعِهِ بِالضَّمِّ حَيْثُ يَنْصَرِفْ",
                "وَاخْفِضْ بِيَاءٍ كُلَّ مَا بِهَا نُصِبْ * وَالْخَمْسَةَ الأَسْمَا بِشَرْطِهَا تُصِبْ",
                "وَاخْفِضْ بِفَتْحِ كُلَّ مَا لَمْ يَنْصَرِفْ * مِمَّا بِوَصْفِ الفِعْلِ صَارَ يَتَّصِفْ",
                "بِأَنْ يَحُوزَ الاُسْمُ عِلَّتَيْنِ * أَوْ عِلَّةً تُغْنِي عَنِ اثْنَتَيْنِ",
                "فَأَلِفُ التَّأْنِيثِ أَغْنَتْ وَحْدَهَا * وَصِيغَةُ الجََمْعِ الَّذِي قَدِ انْتَهى",
                "وَالْعِلَّتَانِ الْوَصْفُ مَعْ عَدْلٍ عُرِفْ * أَوْوَزْنِ فِعْلٍ أَوْ بِنُونٍ وَأَلِفْ",
                "وَهَذِهِ الثَّلاِثُ تَمْنَعُ الْعَلَمْ * وَزَادَ تَرْكِيباً وَأَسْمَاءَ الْعَجَمْ",
                "كَذَاكَ تَأْنِيثٌ بِمَا عَدَا الأَلِفْ * فَإِنْ يُضَفْ أَوْيَأْتِ بَعْدَ أَلْ صُرِفْ"
            ),
            "terjemah" to arrayOf(
                "Tanda-tanda I'rab Jer",
                "Tanda i'rab khafadh/jer yang sudah ditentukan ialah kasrah, ya’ dan fathah.",
                "Khofadhkanlah dengan kasrah kepada isim-isim yang telah diketahui tanda rofa’nya dengan dhommah jika munsharif (menerima tanwin)",
                "Khofadhkanlah dengan ya’ kepada setiap lafadz yang dinashobkan dengan ya’, dan pada isim lima (asmaul khamsah) dengan syarat-syarat yang sudah ditentukan",
                "Khofadhkanlah dengan fathah pada setiap isim yang bertanwin (ghairu munsharif) dari isim yang persifatan dengan sifat fi’il",
                "Isim Ghairu munsharif itu terjadi karena isim itu mempunyai 'illat (sebab/penyakit) dan 'illat itu ada yang dua dan ada yang satu tetapi mencukupi",
                "Alif ta’nits dan sighot muntahal jumu’ adalah 'illat satu yang mencukupi 'illat dua",
                "Adapun 'illat dua yaitu: Sifat bersama ‘adal, Sifat bersama wazan fi’il, dan Sifat bersama tambahan alif nun",
                "Ketiganya (yakni ‘adal, wazan fi’il, dan tambahan alif nun) itu dapat mencegah bersama dengan alam, dan ditambah tarkib mazji, isim ajam",
                "Begitu juga ta’ ta'nits (selain ta’nits bil alif) bila isim ghairu munsharif’ itu dimuddofkan atau jatuh sesudahnya al (ال) maka isim itu menjadi munsharif"
            )
        )
        val babTandaJazm = mapOf(
            "arab" to arrayOf(
                "بَابُ عَلاَمَاتِ الجَزْمِ",
                "والجَزْمُ فِي الأَفْعَالِ بِالسُّكُونِ * أَوْحَذْفِ حَرْفِ عِلَّةٍ أَوْنُونِ",
                "فَحَذْفُ نُونِ الرَّفْعِ قَطْعاً يَلْزَمُ * فِي الخَمْسَةِ الأَفْعَالِ حَيْثُ تُجْزَمُ",
                "وَبِالسُّكُونِ اجْزِمْ مُضَارِعاً سَلِمْ * مِنْ كَوْنِهِ بِحَرْفِ عِلَّةٍ خُتِمْ",
                "إمَّا بِوَاوٍ أَوْ بِيَاءٍ أَوْ أَلِفْ * وَجَزْمُ مُعْتَلٍّ بِهَا أَنْ تَنْحَذِفْ",
                "وَنَصْبُ ذِي وَاوٍ وَيَاءٍ يَظْهَرُ * وَمَا سوَاهُ فِي الثَّلاَثِ قَدَّرُوا",
                "فَنَحْوُ يَغْزُو يَهْتَدِي يخْشى خُتِمْ * بِعِلَّةٍ وغَيْرُهُ مِنْهَا سَلِمْ",
                "وَعِلَّةُ الأَسْمَاءِ يَاءٌ وَأَلِفْ * فَنَحْوُ قَاضٍ والْفَتَى بِهَا عُرِفْ",
                "إِعْرَابُ كُلٍّ مِنْهُمَا مُقَدَّرَ * فِيهَا وَلكِنْ نَصْبُ قاضٍ يَظْهَرَ",
                "وَقَدَّرُوا ثَلاَثَةَ الأَقْسَامِ * فِي الْمِيمِ قَبْلَ الْيَاءِ مِنْ غُلاَمِي",
                "وَالْوَاوُ فِي كَمُسْلِمِيَّ أُضْمِرَتْ * وَالنُّونُ في لَتُبْلَوُنَّ قُدِّرَتْ"
            ),
            "terjemah" to arrayOf(
                "Bab Tanda-tanda Irab Jazem",
                "I’rob Jazm di dalam beberapa fi'il itu dengan sukun atau membuang huruf ‘illat atau membuang nun tanda rofa’",
                "Membuang nun tanda rofa’ adalah wajib pada af'alul khomsah ketika dijazmkan",
                "Jazmkanlah dengan sukun fi’il mudhari’ yang huruf akhirnya selamat dari huruf 'illat",
                "Baik yang berupa Wawu, Ya’, atau alif, Fi’il mu’tal akhir ketika Jazm itu diberi tanda dengan membuang huruf illat",
                "Nashobnya Fi'il mu’tal wawu dan ya’ itu dhohir (jelas), Adapun fi’il mudhori’ mu’tal wawu, ya’, dan alif ketika rofa’ diberi tanda dengan dhommah muqaddarah (dikira-kirakan)",
                "contoh seperti: يَغْزُو يَهْتَدِي يخْشى adalah fi’il yang akhirnya berupa huruf 'illat, Adapun fi’il yang akhirnya tidak berupa 'illat maka dinamakan fi’il shahih",
                "Huruf 'illat yang masuk pada kalimah isim itu hanya ya’ dan alif , seperti lafaz: قَاضٍ dan الْفَتَى",
                "I’rob keduanya yaitu isim mu’tal ya’ (yakni isim manqush) dan mu'tal alif (yakni isim maqshur) itu muqaddar (dikira-kirakan) ketika rofa’, nashob, dan jer. Tetapi isim manqush ketika nashob i'robnya jelas.",
                "Ahli Nahwu sama mengira-ngirakan i’rob pada mim sebelum ya' dari lafadz غُلاَمِي (isim shahih yang kemasukan ya’ mutakkallim)",
                "Wawu yang ada didalam lafadz مُسْلِمِيَّ itu disimpan Dan nun yang ada didalam lafadz لَتُبْلَوُنَّ itu dikira-kirakan (muqaddaroh)."
            )
        )
        val fasal = mapOf(
            "arab" to arrayOf(
                "فَصْلٌ",
                "المُعْرَبَاتُ كُلُّهَا قَدْ تُعْرَبُ * بِالْحَرَكَاتِ أَوْ حُرُوفٍ تَقْرُبُ",
                "فَأَوَّلُ الْقِسْمَيْنِ مِنْهَا أَرْبَعُ * وَهْيَ الَّتِي مَرَّتْ بِضَمٍّ تُرْفَعُ",
                "وَكُلُّ مَا بِضَمَّةٍ قَدِ ارْتَفَعْ * فَنَصْبُهُ بِلْفَتْحِ مُطْلَقاً يَقَعْ",
                "وَخَفْضُ الاِسْمِ مِنْهُ بِالْكَسْرِ الْتُزِمْ * وَالْفِعْلُ مِنْهُ بِاالسُّكُونِ مَنْجَزِمْ",
                "لكِنْ كَهِنْدَاتٍ لِنَصْبِهِ انْكَسَرْ * وَغَيْرُ مَصْرُوفٍ بِفَتْحَةٍ يُجَرّ",
                "وَكُلُّ فِعْلٍ كَانَ مُعْتَلاٌّ جُزِمْ * بِحَذْفِ حَرْفِ عِلَّةٍ كَمَا عُلِمْ",
                "وَالمُعْرَبَاتُ بِالحُرُوفِ أَرْبَعُ * وَهْيَ المُثَنَّى وَذُكُورٌ تُجْمَعُ",
                "جَمْعاً صَحِيحاً كَالْمِثَالِ الخَالِي * وَخَمْسَةُ الأَسْمَاءِ وَالأَفْعَالِ",
                "أَمَّا المُثَنَّى فَلِرَفْعِهِ الأَلِفْ * وَنَصْبُهُ وَجَرُّهُ بِالْيَا عُرفْ",
                "وَكَالْمُثَنَّى الجَمْعُ فِي نَصْبٍ وَجَرّ * وَرَفْعُهُ بِالْوَاوِ مَرَّ وَاسْتَقَرّ",
                "وَالْخَمْسَةُ الاسْمَا كَهَذَا الجَمْعِ فِي * رَفْعٍ وَخَفْضٍ وَانْصِبَنْ بِالأَلِفِ",
                "وَالْخَمْسَةُ الأَفْعَالُ رَفْعُهَا عَرِفْ * بِنونِهَا وَفِي سوَاهُ تَنْحَذِفْ"
            ),
            "terjemah" to arrayOf(
                "Fasal",
                "Semua kalimah yang mu’rob itu ada kalanya di i’robi dengan harakat dan ada pula yang di i’robi dengan huruf",
                "Bagian pertama dari dua bagian tersebut (yaitu yang di i’robi dengan harakat), ada empat macam. Yaitu kalimah yang ketika rofa’ di i’robi dengan dhommah",
                "Semua kalimah yang dirofa’kan dengan dhommah maka nashobnya dengan fathah",
                "Jer dengan kasrah. jazm dengan sukun",
                "Tetapi seperti هِنْدَاتٍ (Jama’ mu-annats salim) nashobnya  dengan kasrah, dan isim ghairu munshorif ketika jer ditandai dengan harakat fathah",
                "Semua fi’il mu’tal itu dijazmkan dengan membuang huruF ’illat, sebagaimana telah diterangkan",
                "Kalimah yang di i’robi dengan huruf itu terbagi menjadi empat bagian: Isim tatsniyah,",
                "Jama’ mudzakkar salim, seperti contoh yang telah lewat Asmaul khomsah, Af‘alul khomsah",
                "Isim tatsniyah ketika tingkah rofa’ diberi tanda dengan alif, dan ketika nashob dan jer diberi tanda dengan ya’",
                "Jama’ mudzakkar salim ketika nashob dan jernya seperti isim tatsniyah (yakni dengan ya’), Sedangkan rofa’nya selamanya dengan wawu",
                "Adapun i’robnya asmaul khomsah itu seperti i’robnya jama’ mudzakkar salim ketika rofa’ dan jernya, dan ketika nashob dengan alif",
                "Adapun i’robnya af‘alul khomsah ketika rofa’ dengan nun, ketika nashob dan jazm dengan membuang nun"
            )
        )
        val babMarifatNakirah = mapOf(
            "arab" to arrayOf(
                "بَابُ المَعْرِفَةِ وَالنَّكِرةِ",
                "وَإِنْ تُرِدْ تَعْرِيف الاِسْمِ النَّكِرَهْ * فَهْوَ الَّذي يَقْبَلُ أَلْ مَؤَثِّرَهْ",
                "وَغَيرُهُ مَعَارِفٌ وتُحْصَرُ * فِي سِتَّةٍ فَالأَوَّلُ مُضْمَرُ",
                "يُكْنَى بِهِ عَنْ ظَاهِرٍ فَيَنْتَمِي * لِلْغَيْبِ والْحُضُورِ والتَّكَلمِ",
                "وَقَسَّمُوهُ ثَانياً لِمُتَّصِلْ * مُسْتَتِرٍ أَوْبَارِزٍ أَوْ مُنْفَصِلْ",
                "ثَانِي المَعَارِفِ الشَّهِيرُ بِالْعَلَمْ * كَجَعْفَرٍ وَمَكَّةٍ وَكَالحَرَمْ",
                "وَأُمُّ عَمْرٍو وَأَبي سَعِيدٍ * وَنَحْوِ كَهْفِ الظُّلْمِ وَالرَّشَيْدِ",
                "فَمَا أَتَى مِنْهُ بِأُمٍّ أَوْ بِأَبْ * فَكُنْيَةٌ وَغَيْرُهُ اسْمٌ أَوْ لَقَبْ",
                "فَمَا بِمَدْحٍ أَوْ بِذَمٍّ مُشْعِرُ * فَلَقَبٌ وَالاِسْمُ مَا لاَ يُشْعِرُ",
                "ثَالِثُهَا إِشَارَةٌ كَذَا وَذِي * رَابِعُهَا مَوْصُولُ الاِسْمِ كَالَّذِي",
                "خَامِسُهَا مُعَرَّفٌ بِحَرْفِ أَلْ * كَمَا تَقُولُ فِي مَحَلِّ المَحَلْ",
                "سَادِسُهَا مَا كَانَ مِنْ مُضَافِ * لِوَاحِدٍ مِن هذِهِ الأَصْنَافِ",
                "كَقَوْلِكَ ابْنِي وَابْنُ زَيْدٍ وَابْنُ ذِي * وَابْنُ الَّذِي ضَرَبْتُهُ وَابْنُ الْبَذِي"
            ),
            "terjemah" to arrayOf(
                "Bab Ma'rifat dan Nakirah",
                "Jika kamu ingin mengetahui isim nakiroh, isim itu adalah isim yang dapat menerima Al yang menjadikan ma’rifat",
                "Selain isim nakirah yaitu isim ma’rifat, dan isim ma’rifat itu terdiri dari enam perkara, Yang pertama isim dhomir,",
                "Isim dhomir adalah isim yang menggantikan isim dzhahir, dan dinashobkan kepada ghaib, hadhir dan mutakallim.",
                "Dhomir dibagi menjadi dua bagian, yaitu Muttashil dan Munfashil. Muttashil dibagi menjadi dua, yaitu Mustatir (tersembunyi) dan Bariz (terang).",
                "Isim ma’rifat yang kedua ialah isim 'alam. Contohnya seperti lafadz: عْفَرٍ, وَمَكَّةٍ, الحَرَمْ",
                "أُمُّ عَمْرٍو, أَبي سَعِيد, نَحْوِ كَهْفِ الظُّلْم,ِ الرَّشَيْدٍ",
                "'Alam Kunyah ialah 'alam yang datangnya dimulai dengan lafadz atau Adapun lainnya alam isim dan alam laqab.",
                "'Alam laqob ialah 'alam yang mengandung pujian atau cacian",
                "Isim ma’rifat yang ketiga ialah isim isyaroh seperti ذَا dan ذِي, Isim Ma'riat yang keempat adalah isim mausul seperti الَّذِي",
                "Isim ma’ifat yang kelima ialah isim yang kemasukkan al (ال) ma'rifat, Seperti kau ucapkan مَحَلِّ menjadi المَحَلْ",
                "Isim ma’rifat yang ke enam ialah isim yang dimudhofkan dengan salah satu dari isim ma’rifat",
                "Contohnya: ابْنِي (Yang dimudlhofkan kepada dhomir), ابْنُ زَيْدٍ (Yang dimudhofkan kepada alam), ابْنُ ذِي (Yang dimudhofkan kepada isim isyaroh), ابْنُ الَّذِي ضَرَبْتُهُ (Yang dimudhofkan kepada isim maushul), ابْنُ الْبَذِي (Yang dimudhofkan kepada isim yang kemasukan ال)"
            )
        )
        val babFiil = mapOf(
            "arab" to arrayOf(
                "بَابُ الأَفْعَالِ",
                "أَفْعَالُهُمْ ثَلاَثَةٌ فِي الوَاقِعِ * مَاضٍ وَفِعْلُ الأَمْرِ وَالْمُضَارعِ",
                "فَالْمَاضِ مَفْتُوحُ الأَخَيرِ إِنْ قُطِعْ * عَنْ مُضْمَرٍ مُحرَّكٍ بِهِ رُفِعْ",
                "فَإنْ أَتى مَعْ ذَا الضَّمِيرُ سُكِّنَا * وَضَمُّهُ مَعْ وَاوِ جَمْعٍ عُيِّنَا",
                "وَالاْمْرُ مَبْنِيٌّ عَلَى السُّكونِ * أَوْحَذْف حَرْفِ عِلَّةٍ أَوْ نُونٍ",
                "وَافْتَتِحُوا مُضَارِعاً بِوَاحِدِ * مِنَ الحُرُوفِ الأَرْبَعِ الزَّوَائِدِ",
                "هَمْزٌ وَنُونٌ وَكَذَا يَاءٌ وتَا * يَجْمَعُهَا قَوْلِي أَنَيْتُ يَافَتَى",
                "وحَيْثُ كَانَتْ فِي رُبَاعِيٍّ تُضَمّ * وَفَتْحُهَا فِيما سِوَاهُ مُلْتَزَمْ"
            ),
            "terjemah" to arrayOf(
                "Bab Fi'il-fi'il",
                "Menurut ahli nahwu Fi’il ada tiga, yaitu Fi’il madhi, fi’il amr, dan fi’il mudhori’",
                "Fi'il madhi itu selalu difathahkan huruf akhirnya, jika lepas dari dhomir mutaharrik yang dirofa’kan",
                "Jika fi’il madhi bertemu dengan dhomir rofa’ mutaharrik maka dibaca sukun, jika bertemu dengan wawu jama’ maka tentu dibaca dhommah",
                "Fi’il amr dimabnikan sukun atau membuang huruf ‘illat atau nun",
                "Para ahli nahwu mengawali fi’il mudhori’ dengan salah satu empat huruf zaidah",
                "yaitu hamzah dan nun, demikian pula ya’ dan ta’ yang terhimpun pada lafadz أَنَيْتُ يَافَتَى (Engkau telah mendekatkan diri, Wahai pemuda!)",
                "Sekiranya fi’il mudhori’ berbentuk رُبَاعِيٍّ maka huruf mudhora’ahnya dibaca dhommah, sedangkan jika tidak berbentuk رُبَاعِيٍّ maka huruf mudhora’ahnya wajib dibaca fathah"
            )
        )
        val babIrabFiil = mapOf(
            "arab" to arrayOf(
                "بَابُ إِعْرَابِ الْفِعْلِ المضارع",
                "رَفْعُ المُضَارعِ الَّذِي تَجَرَّدَا * عَنْ نَاصِبٍ وَجَازِمٍ تَأَبَّدَا",
                "فانْصِبْ بِعَشْرٍ وَهْيَ أَنْ وَلَنْ وَكَيْ * كَذَا إِذَنْ إِنْ صُدِّرَتْ وَلاَمُ كَيْ",
                "وَلاَمْ جَحْدٍ وَكَذَا حَتَّى وَأَوْ * وَالْوَاوُ وَالْفَا فِي جَوَابٍ وَعَنَوْا",
                "بِهِ جَوَاباً بَعْدَ نَفْيٍ أَوْ طَلَبْ * كَلاَ تَرُمْ عِلْماً وتَتْرُكِ التَّعَبْ",
                "وَجَزْمُهُ بِلَمْ وَلَمَّا قَدْ وَجَبْ * وَلاَ وَلاَمٍ دَلَّتَا عَلَى الطَّلَبْ",
                "كَذَاكَ إِنْ ومَا وَمَنْ وَإِذْمَا * أَيٌّ مَتَى أَيَّانَ أَيْنَ مَهْمَا",
                "وَحَيْثُمَا وَكَيْفَمَا وَأَنَّى * كَإِنْ يَقُمْ زَيْدٌ وَعَمْرٌو قمْنَا",
                "وَاجْزِمْ بِإِنْ وَمَا بِهَا قَدْ أُلْحِقَا * فِعْلَيْنِ لَفْظاً أَوْ مَحَلاًّ مُطْلَقَا",
                "وَلْيَقْتَرِنْ بِالْفَا جَوَابٌ لَوْوَقَعْ * بَعْدَ الأَدَاةِ مَوْضِعَ الشَّرْطِ امْتَنَعْ"
            ),
            "terjemah" to arrayOf(
                "Bab I’robnya Fi'il Mudhari'",
                "Rofa’nya fi’il mudhari’ yang sepi dari amil yang menashobkan dan menjazmkan adalah selama-lamanya",
                "Nashobkanlah fi’il mudhari dengan sepuluh amil, yaitu: أَنْ, لَنْ, كَيْ begitu juga إِذَنْ bila dipakai permulaan kata, dan لاَمُ كَي",
                "Lam Juhud, begitu juga حَتَّى dan أَو, dan Wawu jawab dan Fa Jawab",
                "Wawu dan fa’ itu untuk menjawab sesudah kalimah nafi atau tholab (yakni amr atau nahi), seperti lafadz لاَ تَرُمْ عِلْماً وتَتْرُكِ التَّعَب",
                "Adapun jazmnya Fi’il mudhari’ itu ada kalanya dengan لَمْ ,لَمَّا ,لاَ, dan huruf lam  yang menunjukkan thalab",
                "Begitu juga إِنْ, مَا, مَنْ, إِذْمَا, أَيٌّ, مَتَى, أَيَّانَ, أَيْنَ, مَهْمَا",
                "dan juga حَيْثُمَا, كَيْفَمَا, أَنَّى, contohnya إِنْ يَقُمْ زَيْدٌ وَعَمْرٌو قمْنَا",
                "Dan harus jazmkan dengan  إِنْ dan yang disamakan keduanya, kepada dua fi’il baik lafadznya atau mahalnya secara muthlak (mahalnya), yakni tempatnya",
                "Apabila ada jawab, kemudian seumpama dibuat syarat tidak boleh (sah) maka wajib dipasang fa’ jawab."
            )
        )
        val babIsimRofa = mapOf(
            "arab" to arrayOf(
                "بَابُ مَرْفَوعَاتِ الأَسْمَاءِ",
                "مَرْفُوعُ الاسْمَا سَبْعَةٌ نَأْتي بِهَا * مَعْلُومَةَ الأَسْمَاءِ مِنْ تَبْوِيبِهَا",
                "فَالْفَاعِلُ اسْمٌ مُطْلَقاً قَدِ ارْتَفَعْ * بِفِعْلِهِ وَالْفِعْلُ قَبْلَهُ وَقَعْ",
                "وَوَاجِبٌ فِي الْفِعْلِ أَنْ يُجَرَّدَا * إِذَا لِجَمْعٍ أَوْ مُثَنَّى أُسْنِدَا",
                "فَقُلْ أَتَى الزَّيْدَانِ وَازَّيْدُونَا * كَجَاءَ زَيْدٌ وَيَجِي أَخُونَا",
                "وَقَسَّمُوهُ ظَاهِراً وَمُضْمَرَاً * فَالظَّاهِرَ اللَّفْظُ الَّذِي قَدْ ذكِرَا",
                "وَالمُضْمَرُ اثْنَا عَشَرَ نَوْعاً فُسِّمَا * كَقُمْتُ قُمْنا قُمْتَ قُمْتِ قُمْتُمَا",
                "قُمْتُنَّ قُمْتُمْ قَامَ قَامَتْ قاما * قَامُوا وَقُمْنَ نَحْوُ صُمْتُمْ عَامَا",
                "وَهَذِهِ ضَمَائرٌ مُتَّصِلَهْ * وَمِثْلُهَا الضَّمَائِرُ المُنْفَصِلَهْ",
                "وَغَيْرُ ذَيْنِ بِالْقِيَاسِ يُعْلَمُ * كَلَمْ يٍقُمْ إِلاَّ أَنَا أَوْ أَنْتُمُ"
            ),
            "terjemah" to arrayOf(
                "Bab isim-isim yang dibaca Rofa'",
                "Isim-isim yang dirofa’kan itu ada tujuh, Adapun nama-namanya akan diterangkan dalam bab-babnya sendiri-sendiri",
                "Fa'il ialah isim yang dirofa’kan dengan muthlak oleh fi’ilnya, dan fi'il itu terletak sebelumnya",
                "fi’il wajib sepi dari tanda tatsniyah dan jama, bila disandarkan kepada fa’il jama’ atau tatsniyah.",
                "contoh أَتَى الزَّيْدَانِ وَازَّيْدُونَا, جَاءَ زَيْدٌ ,يَجِي أَخُونَا",
                "Ahli nahwu telah membagi fa’il menjadi Isim dhomir dan isim dzahir, Isim dzahir ialah lafadz yang telah diterangkan di atas",
                "Adapun isim dhomir itu terbagi menjadi 12 macam, seperti قُمْتُ, قُمْنا, قُمْتَ, قُمْتِ, قُمْتُمَا",
                "Lalu قُمْتُنَّ, قُمْتُمْ, قَامَ, قَامَتْ, قاما, قَامُوا, قُمْنَ contohnya صُمْتُمْ عَامَا",
                "Ini adalah dhomir-dhomir muttasil, seperti dhomir muttasil adalah dhomir munfasil",
                "Dan selain yang dua macam ini bisa diketahui dengan qiyas, Seperti لَمْ يٍقُمْ إِلاَّ أَنَا أَوْ أَنْتُم"
            )
        )
        val naibulFail = mapOf(
            "arab" to arrayOf(
                "بَابُ نَائِبِ الْفَاعِلِ",
                "أَقِمْ مَقَامَ الْفَاعِلِ الَّذِي حُذِفْ * مَفْعُولَهُ فِي كُلِّ مَالَهُ عُرِفْ",
                "أَوْ مَصْدَراً أَوْ ظَرْفاً أَوْ مَجْرُورَا * إِنْ لَمْ تَجِدْ مَفْعُولَهُ المَذْكُورَا",
                "وَأَوَّلُ الْفِعْلِ الَّذِي هُنَا يُضَمّ * وَكَسْرُ مَاقَبْلَ الأَخَيْرِ مُلْتَزَمْ",
                "فِي كُلِّ مَاضٍ وَهْوَ فِي المُضَارعِ * مُنْفَتِحٌ كَيُدَّعَى وَكَادُّعِي",
                "وَأَوَّلُ الفِعْلِ الَّذِي كَبَاعَا * مُنْكَسِرٌ وَهْوَ الَّذِي قَدْ شَاعَا",
                "وَذَاكَ إِمَّا مُضْمَرٌ أَوْ مُظْهَرُ * ثَانِيهِمَا كَيُكْرَمُ المُبَشِّرُ",
                "أَمَّا الضَّمِيْرُ فَهْوَ نَحْوُ قَوْلِنَا * دُعِيتُ أُدْعى مَادُعِي إِلاَّ أَنَا"
            ),
            "terjemah" to arrayOf(
                "Bab Naibul Fa'il",
                "Tempatkanlah maf'ul pada tempatnya fa’il yang dibuang dalam semua perkara (i’rob) yang sudah diketahui",
                "Atau tempatkanlah mashdar atau dzorof atau majrur (isim yang dijerkan), bila engkau tidak menjumpai maf’ul yang disebut",
                "Adapun huruf awal fi’il madhi dalam bab naibul fa’il itu dibaca dhommah dan huruf sebelum akhir dibaca kasrah",
                "Adapun dalam Fi’il mudhari’ huruf sebelum akhir dibaca fathah seperti يُدَّعَى dan ادُّعِي",
                "Adapun huruf awal Fi’il yang seperti lafadz (Fi’il mu'tal ‘ain) itu dibaca kasrah, itu yang masyhur, Adapun fi’il mu'tal ain itu dibaca kasrah huruf awwalnya",
                "Adapun naibul fa'il itu ada yang berupa isim dhomir dan ada Yang berupa isim dzohir, Adapun yang berupa isim dzahir seperti lafadz يُكْرَمُ المُبَشِّر",
                "Adapun Dhomir seperti دُعِيتُ أُدْعى, مَادُعِي إِلاَّ أَنَا"
            )
        )
        val babMubtadaKhabar = mapOf(
            "arab" to arrayOf(
                "بَابُ المُبْتَدَإ وَالْخَبَر",
                "الْمُبْتُدَا اسْمٌ رَفْعُهُ مُؤَبَّدُ * عَنْ كَلِّ لَفْظٍ عَامِلٍ مُجَرَّدُ",
                "وَالْخَبَرُ اسْمُ ذُو ارْتِفَاعٍ أُسْنِدَا * مُطَابِقاً فِي لَفْظِهِ لِلْمُبْتَدَا",
                "كَقَوْلِنَا زَيْدٌ عَظَيمُ الشَّانِ * وَقَوْلِنَا الزَّيْدَانِ قائِمَانِ",
                "وَمِثْلُهُ الزَّيْدُونَ قائِمُونَا * وَمِنْهُ أَيْضاً قَائِمٌ أَخُونَا",
                "وَالْمُبْتَدَا اسْمٌ ظَاهِرٌ كَمَا مَضى * أَو مُضْمَرٌ كَأَنْتَ أَهْلٌ لِلقَضَا",
                "وَلاَ يَجُوزُ الاِبْتِدَا بِمَا اتَّصَلْ * مِنَ الضَّمِيرِ بَلْ بِكُلِّ مَا انْفَصَلْ",
                "أَنَا وَنَحْنُ أَنْتَ أَنْتِ أَنْتُمَا * أَنْتُنَّ أَنْتُمْ وَهْوَ وَهْيَ هُمْ هُمَا",
                "وَهُنَّ أَيْضاً فَالجَمِيعُ اثْنَا عَشَرْ * وَقَدْ مَضى مِنْهَا مِثَالٌ مُعْتَبَرْ",
                "وَمُفْرَداً وَغَيْرُهُ يَأْتِي الخَبَرْ * فَالأَوَّلُ اللَّفْظُ الَّذِي فِي النَّظْمِ مَرّ",
                "وَغَيْرُهُ فِي أَرْبَعٍ مَحْصُورُ * لاَ غَيْرُ وَهْيَ الظَّرْفُ وَالْمَجْرُورُ",
                "وَفَاعِلٌ مَعْ فِعْلِهِ الَّذِي صَدَرَ * وَالْمُبْتَدَا مَعْ مَالَهُ مِنَ الخَبَرْ",
                "كَأَنْتَ عِنْدِي وَالْفَتَى بِدَارِي * وَابْنِي قَرَا وَذَا أَبُوهُ قَارِي"
            ),
            "terjemah" to arrayOf(
                "Bab Mubtada' dan Khobar",
                "Adapun mubtada’ ialah isim yang dirofa’kan selamanya, Dan yang disepikan dari amil yang dilafadzkan",
                "Adapun khobar ialah isim yang dirofa’kan (disandarkan) kepada mubtada’, sedangkan lafadznya disesuaikan kepada mubtada’",
                "Seperti زَيْدٌ عَظَيمُ الشَّانِ dan الزَّيْدَانِ قائِمَان",
                "Dan seperti الزَّيْدُونَ قائِمُونَا, juga seperti قَائِمٌ أَخُونَا",
                "Mubtada’ itu ada dua bagian, yaitu isim dzohir seperti contoh yang sudah lewat, dan ada isim dhomir seperti أَنْتَ أَهْلٌ لِلقَضَا",
                "Tidak boleh membuat mubtada’ dengan isim dhomir muttashil tetapi harus dengan dhomir munfashil",
                "أَنَا, نَحْنُ, أَنْتَ, أَنْتِ, أَنْتُمَا, أَنْتُنَّ, أَنْتُمْ, هْوَ, هْيَ, هُمْ, هُمَا",
                "Begitu juga هُنَّ, semuanya dua belas, contoh-contohnya telah lewat",
                "Khobar itu ada dua, yaitu khobar mufrod dan khobar ghairu mufrod, Adapun khobar mufrod yaitu seperti dalam nazham terdahulu",
                "Adapun khobar ghoiru mufrod itu ada empat macam, yaitu dzorof Jer majrur",
                "Fa’il serta fi’ilnya, Mubtada’ serta khabarnya",
                "Seperti: أَنْتَ عِنْدِي, الْفَتَى بِدَارِي, ابْنِي قَرَا, ذَا أَبُوهُ قَارِي"
            )
        )
        val babKeluargaKaana = mapOf(
            "arab" to arrayOf(
                "كان وأخواتها",
                "إِرْفَعْ بِكَانَ المُبْتَدَا اسْماً وَالْخَبَرْ * بِهَا انْصِبَنْ كَكَانَ زَيْدٌ ذَا بَصَرْ",
                "كَذَاكَ أَضْحى ظَلَّ بَاتَ أَمْسى * وَهَكَذَ أَصْبَحَ صَارَ لَيْسَا",
                "فَتِىءَ وَانْفَكَّ وَزَالَ مَعْ بَرِحْ * أَرْبَعُهَا مِنْ بَعْدِ نَفْيٍ تَتَّضِحْ",
                "كَذَاكَ دَامَ بَعْدَ مَا الظَّرْفِيَّهْ * وَهْيَ الَّتِي تَكُونُ مَصْدِرِيَّهْ",
                "وَكُلُّ مَا صَرَّفْتَهُ مِمَّا سَبَقْ * مَنْ مَصْدَرٍ وَغَيْرِهِ بِهِ الْتَحَقْ",
                "كَكُنْ صَدِيقاً لاَ تَكُنْ مُجَافِياً * وَانْظُرْ لِكَوْنِي مُصْبِحاً مُوَافِيا"
            ),
            "terjemah" to arrayOf(
                "Kana dan Saudara-saudaranya",
                "Adapun mubtada’ dimasuki kaana maka harus dibaca rofa’ karena sebagai isimnya, dan khabarnya harus dibaca nashob. Seperti كَانَ زَيْدٌ ذَا بَصَر",
                "Begitu juga أَضْحى, ظَلَّ, بَاتَ, أَمْسى, أَصْبَحَ, صَارَ, لَيْسَا",
                "Kemudian فَتِىءَ, انْفَكَّ, زَالَ, بَرِحْ, keempat ini harus setelah nafi",
                "Demikian pula دَامَ setelah مَا الظَّرْفِيَّه مَصْدِرِيَّهْ",
                "Setiap lafadz yang ditashrif dari fi’ilnya kaana dan saudara-saudaranya berupa mashdar dan lainnya (yakni mudhari’ dan amr), itu sama dengan fi’il madhinya",
                "Contohnya كُنْ صَدِيقاً لاَ تَكُنْ مُجَافِياً وَانْظُرْ لِكَوْنِي مُصْبِحاً مُوَافِيا"
            )
        )
        val babKeluargaInna = mapOf(
            "arab" to arrayOf(
                "إن وأخواتها",
                "تَنْصِبُ إِنَّ المُبْتَدَا اسْماً وَالْخَبَرْ * تَرْفَعُهُ كَإِنَّ زَيْداً ذُو نَظَرْ",
                "وَمِثْلُ إِنَّ أَنَّ لَيْتَ فِي الْعَمَلْ * وَهَكَذَا كَأَنَّ لَكِنَّ لَعَلّ",
                "وَأَكَّدُوا المَعْنَى بِإِنَّ أَنَّا * وَلَيْتَ مِنْ أَلْفَاظِ مَنْ تَمَنَّى",
                "كَأّنَّ لِلتَّشْبِيهِ فِي المُحَاكِي * واسْتَعْمَلُوا لكِنَّ فِي اسْتِدْرَاكِي",
                "وَلِتَرَجٍّ وَتَوَقُّعٍ لَعَلّ * كَقَوْلِهِمْ لَعَلَّ مَحْبُوبِي وَصَلْ"
            ),
            "terjemah" to arrayOf(
                "Inna dan saudara-saudaranya",
                "Inna itu menashobkan mubtada’ sebagai isimnya dan merofa’kan khobarnya, seperti lafadz إِنَّ زَيْداً ذُو نَظَر",
                "Dan seperti inna (dalam pengalamannya) yaitu: إِنَّ, أَنَّ, لَيْتَ, كَأَنَّ, لَكِنَّ, لَعَلّ",
                "Kuatkan makna dengan إِنَّ dan أَنَّا. Lalu لَيْتَ termasuk lafad Tamanni",
                "Kemudian كَأّنَّ untuk tasybih (menyerupakan), لكِنَّ untuk istidrak (penyanggahan)",
                "Lafadz لَعَلّ untuk تَرَجٍّ dan تَوَقُّعٍ, seperti ucapan orang arab لَعَلَّ مَحْبُوبِي وَصَل (andaikan kekasihku sampai)"
            )
        )
        val babKeluargadzonna = mapOf(
            "arab" to arrayOf(
                "ظن وأخواتها",
                "إِنْصِبْ بِظَنَّ المُبْتَدَا مَعَ الْخَبَرْ * وَكُلِّ فِعْلٍ بَعْدَهَا عَلَى الأَثَرْ",
                "كَخِلْتُهُ حَسِبْتُهُ زَعَمْتُهُ * رَأَيْتُهُ وَجَدْتُهُ عَلِمْتُهُ",
                "جَعَلْتُهُ اتَّخَذْتُهُ وَكُلِّ مَا * مِنْ هَذِهِ صَرَّفْتَهُ فَلْيُعْلَمَا",
                "كَقَوْ لِهِمْ ظَنَنْتُ زَيْداً مُنْجِدَا * وَاجْعَلْ لَنَا هَذَا المَكَانَ مَسْجِدَا"
            ),
            "terjemah" to arrayOf(
                "Dzonna dan saudara-saudaranya",
                "Mubtada’ dan khobar yang dimasuki zhonna, kedua-duanya harus nashob. Demikian pula bila dimasuki saudara-saudaranya",
                "Seperti خِلْتُهُ, حَسِبْتُهُ, زَعَمْتُهُ, رَأَيْتُهُ, جَدْتُهُ, عَلِمْتُهُ",
                "Lalu جَعَلْتُهُ, اتَّخَذْتُهُ. Begitu juga semua lafadz yang ditashrif dari fi’il-fi’il tersebut",
                "Seperti kata ulama ظَنَنْتُ زَيْداً مُنْجِدَا, اجْعَلْ لَنَا هَذَا المَكَانَ مَسْجِدَا"
            )
        )
        val babNaat = mapOf(
            "arab" to arrayOf(
                "بَابُ النَّعْتِ",
                "النَّعْتُ إِمَّارَافِعٌ لِمُضْمَرِ * يَعودُ لِلْمَنْعُوتِ أَوْ لِمُظْهَرِ",
                "فَأَوَّلُ الْقِسْمَيْنِ مِنْهُ أَتْبِعِ * مَنْعُوتَهُ مِنْ عَشْرَةٍ لأَرْبَعِ",
                "فِي وَاحِدٍ مِنْ أَوْجُهِ الإِعْرَابِ * مِنْ رَفْعٍ أَوْخَفْضٍ أَوْ انْتِصَابِ",
                "كَذَا مِنَ الإِفْرَادِ وَالتَّذْكِيرِ * وَالضِّدِّ وَالتَّعْرِيفِ وَالتَّنْكِيرِ",
                "كَقَوْلِنَا جَاءَ الْغُلاَمُ الفَاضِلُ * وَجَاءَ مَعْهُ نِسْوَةٌ حَوَامِلُ",
                "وَثَانِي الْقِسْمَيْنِ مِنْهُ أَفْرِدِ * وَإِنْ جَرَى المَنْعُوتُ غَيْرَ مُفْرَدِ",
                "واجْعَلْهُ فِي التَّأنِيثِ وَالتَّذْكِيرِ * مُطَابِقاً لِلْمُظْهَرِ المَذْكَورِ",
                "مِثَالُهُ قَدْ جَاءَ حُرَّتَانِ * مَنْطَلِقٌ زَوْجَاهُمَا الْعَبْدَانِ",
                "وَمِثْلُهُ أَتَى غُلاَمٌ سَائِلَهْ * زَوْجَتُهُ عَنْ دَيْنِهَا المُحْتَاجِ لَهْ"
            ),
            "terjemah" to arrayOf(
                "Bab Na'at",
                "Na’at (sifat) itu terbagi menjadi dua bagian, ada yang merofa’kan isim dhomir yang kembali kepada man’ut (isim yang disifati) atau merofa’kan isim dzohir",
                "Adapun yang pertama mengikuti man’ut dalam empat perkara dari sepuluh perkara",
                "Dalam salah satu wajib i’rob seperti Rofa', jer, atau nashob",
                "Begitu juga mufrod dan mudzakkarnya dan lawan keduanya, ma'rifat nakirahnya",
                "Contohnya جَاءَ الْغُلاَمُ الفَاضِلُ , جَاءَ مَعْهُ نِسْوَةٌ حَوَامِل",
                "Na’at yang kedua (na’at sababi) itu harus mufrod meskipun man’utnya bukan mufrod (yakni tatsniyah dan jama')",
                "Jadikanlah na’at sababi itu cocok dengan isim zhohir dalam mu’annats dan mudzakkarnya.",
                "Seperti contoh قَدْ جَاءَ حُرَّتَانِ مَنْطَلِقٌ زَوْجَاهُمَا الْعَبْدَان",
                "Contohnya أَتَى غُلاَمٌ سَائِلَه زَوْجَتُهُ عَنْ دَيْنِهَا المُحْتَاجِ لَه"
            )
        )
        val babAtaf = mapOf(
            "arab" to arrayOf(
                "بَابُ الْعَطْفِ",
                "وَأَتْبَعوا المَعْطُوفَ بِالْمَعْطوفِ * عَلَيْهِ فِي إِعْرَابِهِ المَعْرُوفِ",
                "وَتَسْتَوِي الأَسْمَاءُ وَالأَفْعَالُ فِي * إِتْبَاعِ كُلِّ مِثْلَهُ إِنْ يُعْطَفِ",
                "بِالْوَاوِ وَالْفَا أَوْ وَأَمْ وَثُمَّ * حَتَّى وَبَلْ وَلاَ وَلَكِنِ أَمَّا",
                "كَجَاءَ زَيْدٌ ثُمَّ عَمْرٌو وَأَكْرِمِ * زَيْداً وَعَمْراً بِاللِّقَا وَالْمَطْعَمِ",
                "وَفِئَةٌ لَمْ يَأْكُلُوا أَويَحْضُرُوا * حَتَّى يَفُوتَ أَوْيَزُولَ المُنْكَرُ"
            ),
            "terjemah" to arrayOf(
                "Bab Ataf",
                "Ahli nahwu mengikutkan ma’thuf (kalimat yang di ’athafkan) kepada ma’thuf ‘alaih (kalimat yang di ’athafkan) dalam i’robnya yang sudah diketahui (yakni rofa’, nashob, jer. jazm)",
                "Sama antara isim dan fi'il dalam hal diikutkan jika di 'atafkan",
                "Dengan wawu, fa, أَو, أَمْ, ثُمَّ, حَتَّى, بَلْ, لاَ, لَكِنِ, أَمَّا",
                "Seperti جَاءَ زَيْدٌ ثُمَّ عَمْرٌو, أَكْرِمِ زَيْداً وَعَمْراً بِاللِّقَا وَالْمَطْعَم",
                "فِئَةٌ لَمْ يَأْكُلُوا أَويَحْضُرُوا حَتَّى يَفُوتَ أَوْيَزُولَ المُنْكَر"
            )
        )
        val babTaukid = mapOf(
            "arab" to arrayOf(
                "بَابُ التَّوكِيدِ",
                "وَجَائِزٌ فِي الاِسْمِ أَنْ يُؤَكَّدَا * فَيَتْبَعُ المُؤَكَّدُ المُؤَكَّدَا",
                "فِي أَوْجُهِ الإِعْرَابِ وَالتَّعْرِيفِ لاَ * مُنَكِّرٍ فَمَنْ مُؤَكَّدٍ خَلاَ",
                "وَلَفْظُهُ المَشْهُورُ فِيهِ أَرْبَعُ * نَفْسٌ وَعَيْنٌ ثُمَّ كُلُّ أَجْمَعُ",
                "وَغَيْرُهَا تَوَابِعٌ لأَجْمِعَا * مِنْ أَكْتَعٍ وَأَبْتَعٍ وَأَبْصَعَا",
                "كَجَاءَ زَيْدٌ نَفْسُهُ وَقُلْ أَرَى * جَيْشَ الأَمِيرِ كُلَّهُ تَأَخَّرَا",
                "وَطفْتُ حَوْلَ الْقَوْمِ أَجْمَعينَا * مَتْبُوعَةً بِنَحْوِ أَكْتَعِينَا",
                "وَإِنْ تُؤَكِّدْ كَلْمَةً أَعَدْتَهَا * بِلَفْظِهَا كَقَوْلِكَ انْتَهَى انْتَهَى"
            ),
            "terjemah" to arrayOf(
                "Bab Taukid",
                "Isim boleh ditaukidi, maka lafadz yang menaukidi harus mengikuti lafadz yang ditaukidinya",
                "Dalam semua bentuk i’rob dan ma’rifatnya, kecuali yang dinakirohkan tidak bisa ditaukidi",
                "Adapun lafadznya taukid yang terkenal itu ada empat نَفْسٌ, عَيْنٌ, كُلُّ, أَجْمَعُ",
                "Semuanya adalah pengikut أَجْمِعَا, seperti اَكْتَعُ, اَبْتَعُ, dan أَبْصَعُ",
                "Seperti زَيْدٌ نَفْسُهُ, أَرَى جَيْشَ الأَمِيرِ كُلَّهُ تَأَخَّرَا",
                "Lafadz طفْتُ حَوْلَ الْقَوْمِ أَجْمَعينَا, diikuti dengan seperti أَكْتَعِينَا",
                "Jika kamu mentaukidi kalimat maka kamu harus mengulangnya seperti انْتَهَى انْتَهَى"
            )
        )
        val babBadal = mapOf(
            "arab" to arrayOf(
                "بَابُ الْبَدَلِ",
                "إِذَا اسمٌ أَوْ فِعْلٌ لِمِثْلِهِ تَلاَ * وَالْحُكْمُ لِلثَّانِي وَعَنْ عَطْفٍ خَلاَ",
                "فَاجْعَلْهُ فِي إِعْرَابِهِ كَالأَوَّلِ * مُنَقِّباً لَهُ بِلَفْظِ الْبَدَلِ",
                "كُلُّ وَبَعْضٌ وَاشْتِمَالٌ وَغَلَطْ * كَذَلِكَ إِضْرَابٌ فًبِالْخَمْسِ انْضَبَطْ",
                "كَجَاءَنِي زَيْدٌ أَخوكَ وأَكَلْ * عِنْدِي رَغِيفاً نِصْفَهُ وَقَدْ وَصَلْ",
                "إِلَيَّ زَيْدٌ عِلْمُهُ الَّذِي دَرَسْ * وَقَدْ رَكِبْتُ الْيَوْمَ بَكْراً الْفَرَسْ",
                "إِنْ قُلْتَ بَكْراً دُونَ قَصْدٍ فَغَلَطْ * أَوْ قُلْتَهُ قَصْداً فَإِضْرَابٌ فَقَطْ",
                "وَالْفِعْلُ مِنْ فِعْلٍ كَمَنْ يُؤْمِنْ يُثَبْ * يَدْخُلْ جِنَاناً لَمْ يَنَلْ فِيهَا تَعَبْ"
            ),
            "terjemah" to arrayOf(
                "Bab Badal",
                "Apabila ada isim atau fi’il jatuh setalah lafadz yang sama, dan hukum untuk yang kedua, serta tidak ada huruf ‘athaf",
                "Maka i’robnya jadikan seperti lafadz yang pertama dan namakan badal",
                "Badal itu dibagi menjadi lima, yaitu Badal kul minal kul, Badal Ba’dh minal kul, Badal isytimal, Badal Gholad, dan Badal idhrab",
                "Adapun contoh-contohnya yaitu جَاءَنِي زَيْدٌ أَخوكَ, اَكَلَ عِنْدِي رَغِيفاً نِصْفَهُ, وَقَدْ وَصَل",
                "إِلَيَّ زَيْدٌ عِلْمُهُ الَّذِي دَرَس, رَكِبْتُ الْيَوْمَ بَكْراً الْفَرَس",
                "Jika kamu mengatakan بَكْراً tanpa sengaja maka dinamakan Gholat, atau kau ucapkan dengan sengaja maka dinamakan idhrob",
                "Adapun badalnya fi’il dari fi’il itu seperti lafadz مَنْ يُؤْمِنْ يُثَب يَدْخُلْ جِنَاناً لَمْ يَنَلْ فِيهَا تَعَب"
            )
        )
        val babIsimNashob = mapOf(
            "arab" to arrayOf(
                "بَابُ مَنْصُوبَاتِ الأَسْمَاءِ",
                "ثَلاَثَةٌ مِنْ سِائِرِ الأَسْمَا خَلَتْ * مَنْصُوبَةٌ وَهَذِهِ عَشْرٌ تَلَتْ",
                "وَكُلُّهَا تَأْتِي عَلَى تَرْتِيبِهِ * أَوَّلُهَا فِي الذِّكْرِ مَفْعُولٌ بِهِ",
                "وَذَلِكَ اسْمٌ جَاءَ مَنْصُوباً وَقَعْ * عَلَيْهِ فِعْلٌ كَاحْذَرُوا أَهْلَ الطَّمَعْ",
                "فِي ظَاهِرٍ وَمَضْمَرٍ قَدِ انْحَصَرْ * وَقَدْ مَضى التَّمْثِيلُ لِلَّذِي ظَهَرْ",
                "وَغَيْرُهُ قِسْمَانِ أَيْضاً مُتَّصِلْ * كَجَاءَنِي وَجَاءَنَا وَمُنْفَصِلْ",
                "مِثَالُهُ إِيَّاي أَوْ إِيَّانَا * حَيَّيْتَ أَكْرِمْ بِالَّذِي حَيَّانَا",
                "وَقِسْ بِذَيْنِ كُلَّ مُضْمَرٍ فُصِلْ * وَبِاللَّذَيْنِ قَبْلَ كُلٍّ مَتَّصِلْ",
                "فَكُلُّ قِسْمٍ مِنْهُمَا قَدِ انْحَصَرْ * مَاجَاءَ مِنْ أَنْوَاعِهِ فِي اثنَيْ عَشَرْ"
            ),
            "terjemah" to arrayOf(
                "Bab isim-isim yang dibaca nashob",
                "Ada tiga macam dari isim-isim yang telah disebutkan (yakni khobarnya kaana, isimnya inna dan maf’ulnya zhonna) dinashobkan, Dan disini ada sepuluh lagi yang dibaca nashob",
                "Semuanya datang sesuai tertibnya, yang pertama adalah maf'ul bih",
                "Maf'ul bih adalah isim yang dibaca nashob yang menjadi sasaran pekerjaan seperti احْذَرُوا أَهْلَ الطَّمَع",
                "Maf'ul bih terjadi menjadi zhohir dan dhomir, telah lewat contoh zhohir",
                "Selain zhohir adalah muttasil seperti جَاءَنِي, جَاءَنَا, dan مُنْفَصِلْ",
                "Contoh munfasil: إِيَّايَ, إِيَّانَا, حَيَّيْتَ أَكْرِمْ بِالَّذِي حَيَّانَا",
                "Dengan ini samakan setiap dhomir munfasil, dan dengan keduanya seperti dhomir muttasil",
                "Macam-macam semua bagian telah tercakup dalam dua belah"
            )
        )
        val masdar = mapOf(
            "arab" to arrayOf(
                "بَابُ المَصْدَرِ",
                "وَإِنْ تُرِدْ تَصْرِيفَ نَحْوِ قامَا * فَقُلْ يَقُومُ ثُمَّ قُلْ قِيَامَا",
                "فَمَا يَجِيءُ ثَالِثاً فَالْمَصْدَرُ * وَنَصْبُهُ بِفِعْلِهِ مُقَدَّرُ",
                "فَإِنْ يُوَافِقْ فِعْلَهُ الَّذِي جَرَى * فِي اللَّفْظِ وَالْمَعْنَى فَلَفْظِيًّا يُرَى",
                "أَوْ وَافَقَ المعْنَى فَقَطْ وَقَدْ رُوِيَ * بِغَيْرِ لَفْظِ الْفِعْلِ فَهْوَ مَعْنَوِي",
                "فَقُمْ قِيَاماً مِنْ قَبِيلِ الأَوَّلِ * وَقُمْ وُقُوفاً مِنْ قَبِيلِ مَا يَلِي"
            ),
            "terjemah" to arrayOf(
                "Bab Masdar",
                "Apabila kamu bermaksud mentashrif lafadz seperti lafadz قامَا maka ucapkanlah  يَقُومُ lalu قِيَامَا",
                "yang ketiga itu adalah mashdar dan nashobnya dengan fi’il yang dikira-kirakan",
                "Jadi mashdar ialah isim yang dinashobkan, yang dalam tashrifah sebagai urutan ketiga",
                "Apabila ada mashdar cocok dengan fi’ilnya dalam maknanya saja, tidak cocok dalam lafadznya, maka itu dinamakan mashdar ma’nawi",
                "Lafadz قُمْ قِيَاماً merupakan contoh yang pertama (yakni mashdar lafzhi), Sedang lafadz قُمْ وُقُوفا merupakan contoh yang kedua (yakni mashdar ma’nawi)."
            )
        )
        val dhorof = mapOf(
            "arab" to arrayOf(
                "بَاب الظَرْفِ",
                "هُوَ اسْمُ وَقْتٍ أَوْ مَكَانٍ انْتَصَبْ * كُلُّ عَلَى تَقْدِيرِ فِي عِنْدَ العَرَبْ",
                "إِذَا أَتَى ظَرْفُ المَكَانِ مُبْهَمَا * وَمُطْلَقاً فِي غَيْرِهِ فَلْيُعْلَمَا",
                "وَالنَّصْبُ بِالْفِعْلِ الَّذِي بِهِ جَرَى * كَسِرْتُ مِيلاً واعْتَكَفْتُ أَشْهُرَا",
                "أَوْ لَيْلَةً أَوْ يَوْماً أَوْ سِنِينَ * أَوْمُدَّةً أَوْ جُمْعَةً أَوْحِبنَا",
                "أَوْ قُمْ صَبَاحاً أَوْ مَسَاءً أَوْسَحَرْ * أَو غُدْوَةً أَو بُكْرَةً إِلى السَّفَرْ",
                "أَوْ لَيْلَةَ الإِْثْنَيْنِ أَوْ يَوْمَ الأَحَدْ * أَو صُمْ غَدَاً أَو سَرْمَدَاً أَو الأَبَدْ",
                "واسْمُ المَكَانِ نَحْوُ سِرْ أَمَامَهْ * أَو خَلْفَهُ وَرَاءَهُ قُدَّامَهْ",
                "يَمِينَهُ شِمَالَهُ تَلْقَاءَهُ * أَوْ فَوْقَهُ أَو تَحْتَهُ إِزَاءَهُ",
                "أَوْ مَعْهُ أَوْ حِذَاءَهُ أَوْ عِنْدَهُ * أَو دُونَهُ أَو قَبْلَهُ أَو بَعْدَهُ",
                "هُنَاكَ ثُمَّ فَرْسَخاً بَرِيدا * وَهَهُنَا قِفْ مَوْقِفاً سَعِيدا"
            ),
            "terjemah" to arrayOf(
                "Bab Zhorof",
                "Zhorof ialah isim yang menunjukkan makna waktu atau tempat yang dinashobkan, Menurut ahli nahwu semuanya menakdirkan makna فِي",
                "Apabila zhorof makan itu datangnya mubham (tidak dijelaskan) dan muthlaq (mubham atau muhtash) pada lainnya (zhorof zaman) maka itu perlu dimaklumi, Jadi nashabnya zhorof makan disyaratkan seperti:",
                "Nashabnya zhorof (zaman/makan) itu dengan fi’ilnya, Sebagaimana contoh di bawah ini: سِرْتُ مِيلاً, اعْتَكَفْتُ أَشْهُرَا",
                "لَيْلَةً, يَوْماً, سِنِينَ, مُدَّةً, جُمْعَةً, حِبنَا",
                "قُمْ صَبَاحا, مَسَاءً, سَحَرْ, غُدْوَةً, بُكْرَةً ِلى السَّفَر",
                " لَيْلَةَ, الإِْثْنَيْنِ, يَوْمَ الأَحَد, صُمْ غَدَاً, سَرْمَدَا, الأَبَد",
                "Adapun isim makan/zhorof makan itu seperti lafadz سِرْ أَمَامَ, خَلْفَ, وَرَاءَ, قُدَّامَ",
                "يَمِينَ, شِمَالَ, تِلْقَاءَ, فَوْقَ, تَحْتَ, إِزَاءَ",
                "مَعْهُ, حِذَاءَهُ, عِنْدَهُ, دُونَهُ, قَبْلَهُ, بَعْدَهُ",
                "هُنَاكَ, فَرْسَخاً بَرِيدا, هَهُنَا قِفْ مَوْقِفاً سَعِيدا"
            )
        )
        val babHal = mapOf(
            "arab" to arrayOf(
                "بَابُ الحَالِ",
                "الحَالُ وَصْفٌ ذو انْتِصَابٍ آتِي * مُفَسِّراً لِمُبْهَمِ الْهَيْآتِ",
                "وَإِنَّمَا يُؤْتَى بِهِ مُنْكَّرَا * وَغَالِباً يُؤْتَى بِهِ مُؤَخَّرا",
                "كَجَاءَ زَيْدٌ رَاكِباً مَلْفُوفا * وَقَدْ ضَرَبْتُ عَبْدَهُ مَكْتوفا",
                "وَقَدْ يَجِيءُ فِي الْكَلاَمِ أَوَّلاَ * وَقَدْ يَجِيءُ جَامِداً مُؤَوَّلا",
                "وَصَاحِبُ الحَالِ الَّذِي تَقَرَّرا * مُعَرَّفٌ وَقَدْ يَجي مُنَكَّرَا"
            ),
            "terjemah" to arrayOf(
                "Bab Hal",
                "Hal ialah sifat yang dinashobkan yang datangnya untuk menjelaskan tingkah yang samar",
                "Sesungguhnya hal itu selalu berupa isim nakiroh dan biasanya diakhirkan",
                "Adapun contohnya itu seperti lafadz جَاءَ زَيْدٌ رَاكِباً مَلْفُوفا, قَدْ ضَرَبْتُ عَبْدَهُ مَكْتوفا",
                "Terkadang hal berada di permulaan kalam, terkadang hal berupa isim jamid yang dapat ditakwil",
                "Shohibul hal itu harus berupa isim ma’rifat, Tapi kadang-kadang ada yang berupa isim nakirah"
            )
        )
        val babTamyiz = mapOf(
            "arab" to arrayOf(
                "بَابُ التَّمْيِيزِ",
                "تَعْرِيفُهُ آسْمٌ ذُو انْتِصَابٍ فَسَّرَا * لِنِسْبَةٍ أَوْ ذَاتِ جِنْسٍ قَدَّرَا",
                "كَانْصَبَّ زَيْدٌ عَرَقاً وَقَدْ عَلاَ * قَدْراً وَلَكِنْ أَنْتَ أَعْلَى مَنْزِلاَ",
                "وَكَاشْتَرَيْتُ أَرْبَعاً نِعَاجَا * أَو اشْتَرَيْتُ أَلْفَ رِطْلٍ سَاجَا",
                "أَوْ بِعْتُهُ مَكِيلَةً أَرُزًّا * أَوْ قَدْرَ بَاعٍ أَوْ ذَرَاعٍ خَزًّا",
                "وَوَاجِبُ التَّمْيِيزِ أَنْ يُنْكَّرَا * وَأَنْ يَكونَ مُطْلَقاً مُؤَخَّرَا"
            ),
            "terjemah" to arrayOf(
                "Bab Tamyiz",
                "Definisi tamyiz ialah yang dinashobkan yang menjelaskan nisbat atau jenis tertentu",
                "Seperti اِنْصَبَّ زَيْدٌ عَرَقاً وَقَدْ عَلاَ قَدْراً وَلَكِنْ أَنْتَ أَعْلَى مَنْزِلاَ",
                "dan juga اِشْتَرَيْتُ أَرْبَعاً نِعَاجَا, اِشْتَرَيْتُ أَلْفَ رِطْلٍ سَاجَا",
                "kemudian  بِعْتُهُ مَكِيلَةً أَرُزًّا, dan قَدْرَ بَاعٍ أَوْ ذَرَاعٍ خَزًّا",
                "Tamyiz wajib nakirah dan harus di akhirkan"
            )
        )
        val babIstisna = mapOf(
            "arab" to arrayOf(
                "بَابُ الاُسْتِثْنَاءِ",
                "أَخْرِجْ بِهِ الْكَلاَمِ مَا خَرَجْ * مِنْ حُكْمِهِ وَكَانَ فِي اللَّفْظِ انْدَرَجْ",
                "وَلَفْظُ الاُسْتِثْنَا الَّذِي قَدْ حَوَى * إِلاَّ وَغَيْراً وَسِوَى سُوىً سَوَا",
                "خَلاَ عَدَا حَاشَا فَمَعْ إِلاَّ انْصِبِ * مَا أَخْرَجَتْ مِنْ ذِي تَمَامٍ مُوجَبِ",
                "كقَامَ كُلُّ الْقَوْمِ إِلاَّ وَاحِدَا * وَقَدْ رأَيْتُ الْقَوْمَ إِلاَّ خَالِدَا",
                "وَإِنْ يَكُنْ مِنْ ذِي تَمَامٍ انْتَفَى * فَأَبْدِلَنْ وَالنَّصْبُ فِيهِ ضُعِّفَا",
                "هَذَا إِذَا اسْتَثْنَيْهُ مِنْ جِنْسِهِ * وَمَا سِوَاهُ حُكْمُهُ بِعَكْسِهِ",
                "كَلَنْ يَقُومَ القَوْمُ إِلاَّ جَعْفَرُ * وَانَّصْبُ فِي إِلاَّ بَعِيراً أَكْثَرُ",
                "وَإِنْ يَكُنْ مِنْ ناقِصٍ فَإِلاَّ * قَدْ أُلْغِيَتْ وَالْعَامِلُ اسْتَقَلاَّ",
                "كَلَمْ يَقُمْ إِلاَّ أَبوكَ أَوَّلاَ * وَلاَ أَرَى إِلاَّ أَخَاكَ مَقْبِلاَ",
                "وَخَفْضُ مُسْتَثْنىً عَلَى الإِطْلاَقِ * يَجُوزُ بَعْدَ السَّبْعَةِ الْبَوَاقِي",
                "وَالنَّصْبُ أّيْضاً جَائِزٌ لِمَنْ يَشَا * بِمَا خَلاَ ومَا عَدَا وَمَا حَشَا"
            ),
            "terjemah" to arrayOf(
                "Bab Istitsna'",
                "Kecualikanlah dari kalam dengan huruf istitsna’ yang keluar dari hukum yang masuk dalam lafadznya",
                "Adapun lafadz istitsna’ ialah إِلاَّ, غَيْراً, سِوَى, سُوىً, سَوَا",
                "Lafadz خَلاَ, عَدَا, dan حَاشَا. Setiap lafaz yang diistitsna’ dengan إِلاَّ harus dinashobkan, bila kalamnya berupa kalam tamm mujab (yakni tidak didahului nafi dan nahi dan serupanya)",
                "Contohnya seperti lafadz كقَامَ كُلُّ الْقَوْمِ إِلاَّ وَاحِدًا (semua kaum berdiri kecuali satu), رأَيْتُ الْقَوْمَ إِلاَّ خَالِدَا (aku melihat kaum kecuali kholid)",
                "Kalau ada istitsna’ itu dari kalam tamm yang menerima nafi, maka harus dijadikan badal dan nashobnya karena istitsna’ itu dianggap dho’if",
                "Begitu itu apabila kamu membuat istitsna’ dari jenisnya mustatsna’ minhu (istitsna’ muttashil) dan lainnya istitsna’ munqothi’ yaitu satu jenis maka hukumnya sebaliknya, yaitu nashob alal istitsna’",
                "seperti ماقامَ القومُ اِلاَّ جَعْفَرُ (semua kaum berdiri keculai jakfar) nashob dalam contoh إِلاَّ بَعِيرا (kecuali onta) adalah banyak",
                "Kalau ada istitsna’ dari kalam naqish (yang tidak ada mustatsna minhunya), maka lafaz إِلاَّ diilgha’kan (tidak beramal), dan amilnya berdiri sendiri (yakni beramal pada mustatsna)",
                "seperti كَلَمْ يَقُمْ إِلاَّ أَبوكَ (tiiada yang berdri kecuali ayahmu) لاَ أَرَى إِلاَّ أَخَاكَ مَقْبِلا (aku tidak melhat kecuali saudaramu)",
                "Boleh membaca jer mustatsna’ sesudah tujuh lafadz istitsna’ (خَلاَ, عَدَا, خَاشَا, سِوَى, سُوىً, سَوَا dan غَيْراً) secara muthlaq",
                "Nasاob juga boleh bagi yang menghendaki istitsna dengan خَلاَ, مَاعَدَا dan مَاحَاشَا"
            )
        )
        val babLaBeramalaInna = mapOf(
            "arab" to arrayOf(
                "بَابُ لاَ الْعَامِلَةِ عَمَلَ إِنَّ",
                "وَحُكْمُ لاَ كَحُكْمِ إِنَّ فِي الْعَمَلْ * فَانْصِبْ بِهَا مُنَكَّراً بِهَا اتَّصَلْ",
                "مُضَافاً أَوْ مُشَابِهَ الْمُضَافِ * كَلاَ غُلاَمَ حَاضِرٌ مكَافِي",
                "لَكِنْ إِذَا تَكَرَّرَتْ أَجْرَيْتَهَا * كَذَاكَ فِي الأَعْمَالِ أَوْ أَلْغَيْتَهَا",
                "وَعِنْدَ إِفْرَادِ اسْمِهَا الْزَمِ الْبِنَا * مُرَكَّبَا أَوْ رفْعَهُ منَوِّنَا",
                "كَلاَ أَخٌ وَلاَ أَبٌ وَانْصِبْ أَبَا * أَيْضاً وَإِنْ تَرِفَعْ أَخاً لاَ تَنْصِبَا",
                "وَحَيْثُ عَرَّفْتَ اسْمَهَا أَوْ فُصِلاَ * فَارْفَعْ وَنَوِّن وَالْتَزِمْ تَكْرَارَ لاَ",
                "كَلاَ عَلِيٌّ حَاضِرٌ وَلاَ عُمَرْ * وَلاَ لَنَا عَبْدٌ وَلاَ مَا يُدَّخَرْ"
            ),
            "terjemah" to arrayOf(
                "Bab la  yang beramal seperti inna",
                "Adapun hukumnya لَا itu seperti hukumnya Inna dalam amalnya, maka nashobkan dengan لَا, isim nakirah yang bersanding dengannya",
                "Baik itu berupa isim mudlaf atau serupanya mudlaf. Seperti la Gulama hadiron mukafi (tiada anak yang hadir yang membalas)",
                "Tapi bila لَا diulang-ulang maka boleh beramal boleh tidak",
                "Dan jika isimnya لَا berupa isim mufrod, maka harus dimabnikan atau dirofa’kan dengan tanwin",
                "Seperti لاَ أَخٌ وَلاَ أَب, nasobkan أَبا juga, jika kamu merofa'kan أَخاً jangan menashobkan أَبا",
                "Dan jika kamu mema’rifatkan isim لاَ atau memisahkan (antara لاَ dan isimnya) maka rafa’kanlah dengan mentanwinkan dan mengulang-ngulangi لاَ",
                "Contohnya لاَ عَلِيٌّ حَاضِرٌ وَلاَ عُمَرْ, لاَ لَنَا عَبْدٌ وَلاَ مَا يُدَّخَر"
            )
        )
        val babMunada = mapOf(
            "arab" to arrayOf(
                "بَابُ النِّدَاءِ",
                "خَمْسٌ تنَادَى وَهْيَ مَفْرَدٌ عَلَم * وَمُفْرَدٌ منَكَّرٌ قَصْداً يُؤَمّ",
                "وَمُفْرَدٌ مُنَكَّرٌ سِوَاهُ * كَذَا المُضَافُ وَالَّذِي ضَاهَاهُ",
                "فَالأَوَّلاَنِ فِيهِمَا الْبِنَا لَزِمْ * عَلَى الَّذِي فِي رَفْعِ كُلِّ قَدْ عُلِمْ",
                "مِنْ غِيرِ تَنْوِينٍ عَلَى الإِطْلاَقِ * وَالنَّصْبُ فِي الثَّلاَثَةِ الْبَوَاقِي",
                "كَيَا عَلِيٌّ يَا غلاَمِي بِي انْطَلِقْ * يَا غَافِلاً عَنْ ذِكْرِ رَبِّهِ أَفِقْ",
                "يَا كَاشِفَ الْبَلْوَى وَيَا أَهْلَ الثَّنَا * وَيَا لَطِيفاً بِالْعبَادِ الْطُفْ بِنَا"
            ),
            "terjemah" to arrayOf(
                "Bab Munada",
                "Ada lima isim yang dapat dijadikan munada, yaitu Mufrod 'alam, Mufrod nakirah maqshurah",
                "Mufrod nakirah ghairu maqshuroh, Mudhof, Syibih mudhof",
                "Adapun yang pertama dan kedua (yakni munada mufrad 'alam dan mufrod nakirah magshudah) itu harus dimabnikan sesuai i’rob rofa’ yang sudah diketahui",
                "tanpa ditanwin secara muthlak, Adapun yang tiga (yakni mufrod nakirah ghairu maqshudah, mudhof dan syibih mudhof) itu dinashobkan",
                "Seperti يَا عَلِيٌّ, يَا غلاَمِي بِي انْطَلِق, يَا غَافِلاً عَنْ ذِكْرِ رَبِّهِ أَفِقْ",
                "يَا كَاشِفَ الْبَلْوَى, يَا أَهْلَ الثَّنَا, يَا لَطِيفاً بِالْعبَادِ الْطُفْ بِنَا"
            )
        )
        val babMafulLiAjlih = mapOf(
            "arab" to arrayOf(
                "بَابُ المَفْعُولِ لأَجْلِهِ",
                "وِالمَصدَرَ انْصِبْ إِنْ أَتَى بَيَانَا * لِعِلَّةِ الْفِعْلِ الَّذِي قَدْ كَانَا",
                "وَشَرْطُهُ اتِّحَادُهُ مَعْ عَامِلِهْ * فيما لَهُ مِنْ وَقْتِهِ وَفَاعِلِهْ",
                "كَقُمْ لِزَيْدٍ اتِّقَاءَ شَرِّهِ * وَاقٍْصِدْ عَلِيَّاً ابْتِغَاءَ بِرِّهِ"
            ),
            "terjemah" to arrayOf(
                "Bab Maful Li Ajlih",
                "Nashobkanlah mashdar itu apabila ia datang untuk menjelaskan “'Ilatnya fi’il yang ada",
                "Adapun syaratnya maful li ajlih ialah harus tunggal dengan amilnya dalam waktunya dan failnya",
                "Seperti ِقُمْ لِزَيْدٍ اتِّقَاءَ شَرِّه, dan وَاقٍْصِدْ عَلِيَّاً ابْتِغَاءَ بِرِّهِ"
            )
        )
        val babMafulMaah = mapOf(
            "arab" to arrayOf(
                "بَابُ المَفْعُولِ مَعَهُ",
                "تَعْرِيفُهُ اسْمٌ بَعْدَ وَاوٍ فَسَّرَا * مَنْ كَانَ مَعْهُ فِعْلُ غَيْرِهِ جَرَى",
                "فَانْصِبْهُ بِالْفِعْلِ الَّذِي بِهِ اصْطَحَبْ * أَوْشِبْهِ فِعْلٍ كَاسْتَوَى المَاوَالخَشَبْ",
                "وَكَالأَمِيرُ قَادِمٌ وَالْعَسْكَرَا * وَنَحْوُ سِرْتُ وَالأَمِيرَ لِلْقُرَى"
            ),
            "terjemah" to arrayOf(
                "Bab Maful Ma'ah",
                "Definisi maf’ul ma‘ah ialah isim yang jatuh sesudah wawu masiyyah (yakni wawu yang bermakna ma‘a) yang menjelaskan sama-sama mengerjakan suatu pekerjaan",
                "Nashobkanlah maful ma‘ah itu dengan fi’il yang menyertainya atau dengan yang serupa fi’il (seperti isim fa’il)",
                "Seperti اَلأَمِيرُ قَادِمٌ وَالْعَسْكَرَا, dan نَحْوُ سِرْتُ وَالأَمِيرَ لِلْقُرَى"
            )
        )
        val babIsimJer = mapOf(
            "arab" to arrayOf(
                "بَابُ مَخْفوضَاتِ الأَسْمَاءِ",
                "خَافِضُهَا ثَلاَثَةٌ أَنْوَاعٌ * الحَرْفُ وَالمُضَافُ وَالإِتْبَاعُ",
                "أَمَّا الحُرُوفُ هَهُنَا فَمِنْ إِلَى * بَاءٌ وَكَافٌ فِي وَلاَمٌ عَنْ عَلَى",
                "كَذَاكَ وَاوٌبَا وَتَاءٌ فِي الحَلِفْ * مُذْ مُنْذُ رُبَّ وَاوُ رُبَّ المُنْحَذِفْ",
                "كَسرْتُ مَنْ مِصْرَ إِلَى الْعِرَاقِ * وَجِئْتُ لِلْمَحْبُوبِ بِاشْتِيَاقِ"
            ),
            "terjemah" to arrayOf(
                "Bab isim-isim yang dibaca jer",
                "Adapun yang mengerjakan isim itu ada tiga macam, yaitu Huruf, Mudhof, dan Isim yang mengikuti isim yang dijerkan",
                "Huruf di sini adalah: مِنْ, إِلَى, بَاءْ, كَافْ, فِي, لاَمْ, عَنْ, عَلَى",
                "Begitu juga wawu, ba', ta' untuk sumpah, مُذْ, مُنْذُ, رُبَّ, wawu رُبَّ yang terbuang",
                "Seperti كَسرْتُ مَنْ مِصْرَ إِلَى الْعِرَاقِ, dan وَجِئْتُ لِلْمَحْبُوبِ بِاشْتِيَاقِ"
            )
        )
        val babIdhofah = mapOf(
            "arab" to arrayOf(
                "بَابُ الإِضَافَةِ",
                "مِنَ المُضَافِ أَسْقِطِ التَّنْوِينَا * أَوْ نُونَهُ كَأَهْلُكُمْ أَهْلُونَا",
                "وِاخْفِضْ بِهِ الاِسْمَ الَّذِي لَهُ تَلاَ * كَقَاتِلاَ غُلاَمَ زَيْدٍ قُتِلاَ",
                "وَهْوَ عَلَى تَقْدِيرِفي أَوْ لاَمِ * أَوْ مِنْ كَمَكْرِ اللَّيْلِ أَو غَلاَمِي",
                "أَوْ عَبْدِ زَيْدٍ أَوْ إِنَا زُجَاجِ * أَوْ ثَوْبِ خَزٍّ أََوكَبَابِ سَاجِ"
            ),
            "terjemah" to arrayOf(
                "Bab Idofah",
                "Buanglah tanwin atau nun dari mudhof, Seperti lafadz أَهْلُكُمْ أَهْلُونَا",
                "Jerkanlah dengan mudhof, isim yang mengikutinya (mudhof ilaih), Seperti lafadz َقَاتِلاَ غُلاَمَ زَيْدٍ قُتِلا",
                "Idhofah mengira-ngirakan فِي atau lam atau مِنْ seperti مَكْرِ اللَّيْلِ أَو غَلاَمِي",
                "عَبْدِ زَيْدٍ, إِنَا زُجَاجِ, ثَوْبِ خَزٍّ, بَابِ سَاج",
            )
        )
        val akhiran = mapOf(
            "arab" to arrayOf(
                "وَقَدْ مَضَتْ أَحْكَامُ كُلِّ تَابِعِ * مَبْسُوطَةٌ فِي الأَرْبَعِ التَّوَابِعِ",
                "فَيَا إِلَهِي الْطُفْ بِنَا فَنَتَّبِعْ * سُبْلَ الرَّشَادِ وَالْهُدَى فَنَرْتَفِعْ",
                "وَفِي جُمَادَى سَادِسِ السَّبْعِينَا * بَعْدَ انْتِهَا تِسْعٍ مِنَ المِئِينَا",
                "قَدْ تَمَّ نَظْمُ هَذِهِ المُقدِّمَهْ * فِي رُبْعِ أَلْفٍ كَافِيَا مَنْ أَحْكَمَهْ",
                "نَظْمُ الْفَقِيرِالشَّرَفِ الْعَمْرِيطِي * ذِي الْعَجْزِ وَالتَّقْصِيرِ وَالتَّفْرِيطِ",
                "وَالْحَمْدُ لِلَّهِ مَدَى الدَّوَامِ * عَلَى جَزِيلِ الْفَضْلِ وَالإِنْعَامِ",
                "وَأَفْضَلُ الصَّلاَةِ وَالتَّسْلِيمِ * عَلَى النَّبِيِّ المُصْطَفَى الْكَرِيمِ",
                "مُحَمَّدٍ وَصَحْبِهِ وَالآلِ * أَهْلِ التُّقَى وَالْعِلْمِ وَالْكَمَالِ"
            ),
            "terjemah" to arrayOf(
                "Tentang hukumnya tiap-tiap kalimah yang ikut (yakni naat, athaf taukid dan badal) secara panjang lebar sudah diterangkan di atas",
                "Ya Tuhanku, berilah karunia kepada kami maka kami akan mengikuti jalannya petunjuk, Sebab dengan itulah kami dapat naik ketingkat atas",
                "Pada bulan Jumadil Ula, tahun 970 hijriyah",
                "sudah selesai Nadzam yang jumlahnya ada 250, dengan mencukupi orang ingin mempelajari ilmu nahwu",
                "Nadzam Imrithi adalah nadzamnya Al Faqir Asy Syaikh Syarafuddin Yahya Al Irnrithi yang lemah (dari sesuatu) dan teledor (dalam taat) dan menyia-nyiakan (ibadah kepada Allah)",
                "Segala puji bagi Allah selama-lamanya atas baiknya keutamaan dan kenikmatan",
                "Adapun lebih utamanya shalawat dan salam semoga selalu melimpahkan kepada Nabi yang terpilih dan yang mulia,",
                "yaitu Nabi Muhammad, dan semoga melimpah kepada para sahabat beliau, keluarga beliau yang bertaqwa dan berilmu serta penuh dengan, kesempurnaan"
            )
        )
    }
}